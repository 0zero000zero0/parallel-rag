...
ANSWER_TAG_PATTERN = re.compile(r"<answer>(.*?)</answer>",
                                flags=re.DOTALL | re.IGNORECASE)
BOXED_TAG_PATTERN = re.compile(r"<boxed>(.*?)</boxed>",
                               flags=re.DOTALL | re.IGNORECASE)
...
class PromptedGenerationBase:
    ...
    def _make_config(self) -> SimpleNamespace:
        return SimpleNamespace(max_completion_length=self.generation_max_tokens,
...
def build_openai_client_from_args(args,
                                  use_chat_template: bool = False) -> OpenAIClient:
    openai_base_url = getattr(args, "openai_base_url", "http://127.0.0.1:8001")
    openai_api_key = getattr(args, "openai_api_key", "TEST")
    model = getattr(args, "model", None) or getattr(args,
                                                    "llm_model",
                                                    "Qwen3-14B")
