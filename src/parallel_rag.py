...
SEARCH_QUERIES_TAG_PATTERN = re.compile(
    r"<search_queries>(.*?)</search_queries>", flags=re.DOTALL | re.IGNORECASE
)
...
class ParallelRAG(PromptedGenerationBase):
    def __init__(
        ...
        if (
            self.global_refine_agent_use_chat_template
            and self.global_refine_agent_tokenizer is None
        ):
            raise ValueError(
    ...
    def from_args(cls, args) -> "ParallelRAG":
        ...
        if (
            global_refine_agent_use_chat_template
            and global_refine_agent_tokenizer is None
        ):
        ...
        global_refine_agent_top_p = float(
            getattr(
                args, "global_refine_agent_top_p", getattr(args, "refine_top_p", 0.9)
            )
        )
        ...
        user_prompt = "\n\n".join(
            [
                ...
                self._format_external_context(
                    "Historical Refined Information", history_block
                ),
    ...
    def run_batch(
        ...
        for iteration_idx in range(max_iterations):
            ...
            for sample_i in active_indices:
                ...
                results[sample_i]["retrieved_docs"].append(retrieved_for_sample)

                pooled_docs = self._pool_documents(search_queries, docs_map, sample_i)
                ...
                refine_meta.append((sample_i, self._prompt_to_text(refine_prompt)))

            phase2_pooling_ms = self._ns_to_ms(
                self._now_ns() - phase2_pooling_started_ns
            )
