...
chat_ml_str = tokenizer.apply_chat_template(
    chat_ml, tokenize=False, enabel_thinking=True
)
...
def main() -> None:
    ...
    extra_headers = parse_headers(params["headers"]) if params["headers"] else None
    client = OpenAIClient(
    ...
    outputs = client.generate(
        prompts=prompts, config=config, stop_tokens=params["stop_tokens"]
    )
